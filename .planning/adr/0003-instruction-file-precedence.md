...
**Hash.** `bundle_hash` = sha256 over ordered `(path, sha256)` pairs of the files that made the bundle, computed before budget truncation. It is recorded on `SessionRecord` at creation and on every `RunRecord`; later on the agent registry row (S4) and as a BOS4 decision feature (S9).
